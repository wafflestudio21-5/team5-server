...
class CustomOAuth2SuccessHandler(
    ...
    override fun onAuthenticationSuccess(
        ...
        val accessToken = jwtUtils.generateAccessToken(oAuth2User.username)
        ...
        if (oAuth2User.isExistingUser) {
            ...
            val uri = addQueryParameter(SUCCESS_REDIRECT_URI, accessToken, "success")
            redirectStrategy.sendRedirect(request, response, uri)
        } else {
            addRefreshToken(response, refreshToken, "/api/v1/auth/facebook_signup")
            val uri = addQueryParameter(REGISTER_REDIRECT_URI, accessToken, "register")
            redirectStrategy.sendRedirect(request, response, uri)
        }
    }
    ...
    private fun addQueryParameter(
        ...
        return "$baseUri?accessToken=$accessToken&result=$resultValue"
    }
}